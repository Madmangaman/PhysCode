# Python Solar System Sim Earth-Jupiter Interaction;
import math 
import cmath
import numpy as np
    ...
    for count in range(int(maxnsteps/dt)):
        ...
        jupiter_acceleration_vector = np.array([-((G*MSUN*jupiter_pos[0])/(jupitersun_distance**3))-(
                G*MJUPITER*(jupiter_pos[0]-earth_pos[0])/(jupiterearth_distance**3)), -(
                    G*MSUN*jupiter_pos[1])/(jupitersun_distance**3)-(G*MJUPITER*(jupiter_pos[1]-earth_pos[1])/(jupiterearth_distance**3))])
